...
if result == TriBool.TRUE:
    ...
    handle_false()
# Что если UNKNOWN? Код упадёт или поведёт себя непредсказуемо!
```

### 3. Используйте иммутабельные операции

```python
# ✅ Правильно - add возвращает НОВЫЙ объект
base_conds = Conditions()
extended_conds = base_conds.add(new_condition)

# ❌ Неправильно - add НЕ изменяет исходный объект
conds = Conditions()
...
for ctx in contexts:
    engine = ForcingEngine(net)  # Создаётся каждый раз!
    result = engine.forces(ctx, phi, conds)
```
