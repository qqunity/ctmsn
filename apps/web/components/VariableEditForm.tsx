...
import { GraphPayload, UserVariableInfo } from "@/lib/types";

type Props = {
  variable: UserVariableInfo;
  graph: GraphPayload | null;
  onSubmit: (data: { name?: string; type_tag?: string; domain_type?: string; domain?: Record<string, any> }) => void;
  onCancel: () => void;
};

export function VariableEditForm({ variable, graph, onSubmit, onCancel }: Props) {
  const [name, setName] = useState(variable.name);
  const [typeTag, setTypeTag] = useState(variable.type_tag ?? "");
  const [domainType, setDomainType] = useState<"enum" | "range" | "predicate">(variable.domain_type);
  const [enumValues, setEnumValues] = useState<string[]>(variable.values ?? variable.domain?.values ?? []);
  const [enumInput, setEnumInput] = useState("");
  const [rangeMin, setRangeMin] = useState(variable.min ?? variable.domain?.min ?? 0);
  const [rangeMax, setRangeMax] = useState(variable.max ?? variable.domain?.max ?? 100);
  const [rangeInclusive, setRangeInclusive] = useState(variable.domain?.inclusive ?? true);
  const [predName, setPredName] = useState(variable.domain?.name ?? "custom");
  ...
  return (
    <div className="border rounded p-3 bg-yellow-50 space-y-2">
      <div className="text-xs font-semibold text-yellow-700 mb-1">Редактирование: {variable.name}</div>
      ...
      <div className="flex gap-3 text-xs">
        {(["enum", "range", "predicate"] as const).map((dt) => (
          <label key={dt} className="flex items-center gap-1">
            <input
              type="radio"
              name={`editDomainType-${variable.id}`}
      ...
      <div className="flex gap-2 justify-end">
        ...
        <button onClick={handleSubmit} disabled={!name.trim()} className="text-xs bg-yellow-600 text-white px-3 py-1 rounded disabled:opacity-50">
          Сохранить
        </button>
      </div>
    </div>
