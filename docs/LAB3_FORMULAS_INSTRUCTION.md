...
1. Создайте формулу AND, в которой один из атомов содержит **неназначенную переменную** (`student` или `teacher`):
   - `FactAtom("teaches", ivanov, course_db)` — TRUE (факт существует, все аргументы — конкретные концепты)
   - `FactAtom("enrolled_in", student, course_db)` — UNKNOWN (переменная `student` не назначена в контексте)
2. Вычислите — ожидается результат **UNKNOWN** (T ∧ U = U по таблице истинности Клини).
3. Объясните результат: значение UNKNOWN возникает потому, что переменная `student` не имеет назначенного значения в текущем контексте, и `Evaluator` не может определить истинность факта `enrolled_in(student, course_db)`.
